Threaded Project:  PHP/mySQL Prototype WebSite    22 Nov 2013
...
***IMPORTANT:
  ...
  2. Please test using Google Chrome.  There is a single known issue
     with Firefox where clicking ORDER does not pass the  package name.
	 Many features are not supported by Internet Explorer 8.

We feel we have all contributed equally to the presentation
...
Regarding testing:
  ...
  Ordering which leads to booking:
    ...
    -if a logged in customer clicks an order button then
       then go directly to the booking page (called payment.php)
	   where they provide further booking information
       and when they click the booking button then a row is inserted in the database
...
Implemented featuers:
  ...
  -if an agent is logged in then the following pages are available:
    -agent managemant supports creating, displaying, updating and deleting
	-customer management supports displaying
	-supplier management is not implemented
	-report generation display two simple sample reports
